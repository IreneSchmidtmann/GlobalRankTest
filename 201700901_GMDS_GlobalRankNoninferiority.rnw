...
\item Determine non-parametric margin $\tilde{\epsilon}$ without censoring by death such that\\
      ...
      $\Rightarrow \tilde{\epsilon} \approx $ \Sexpr{round(0.5 - pnorm(-1/sqrt(8)), 4)}, \item Determine margin $\epsilon$ for global rank test
\begin{eqnarray*}
...
\begin{frame}[label=slide12,fragile] %slide 12
\frametitle{Application to clinical problem - Dependence of power on sample size and probability of death}
<<powerplot2, echo=FALSE, eval=TRUE,out.width='7cm',out.height='7cm'>>=
source("PowerAndMoments.R") #include functions to compute moments and power
# explore power for specific situation described above, varying the
# probabiliy of death
# sample size in each group
...
legend("bottomright", title = TeX('$p_1 = RR*p_0$'), 
       legend = as.character(chosen_ps), inset = 0.05, bty = "n", lwd = 2, 
       lty = my.lt, col = my.col )
title(main = "Power and sample size by probality of censoring by death",
      sub = "(1:2 allocation)")
