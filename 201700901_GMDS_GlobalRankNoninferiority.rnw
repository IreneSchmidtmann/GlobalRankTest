...
%\includeonlyframes{slide12,slide13}
...
\begin{frame}[label = slide11b,fragile] %slide 11b
...
\end{eqnarray*}
      ...
      $\Rightarrow \tilde{\epsilon} \approx $ \Sexpr{round(0.5 - pnorm(-1/sqrt(8)), 4)}
\end{frame}

\begin{frame}[label = slide11a,fragile] %slide 11a
...
\begin{frame}[label=slide12a,fragile] %slide 12a
\frametitle{Application to clinical problem - Dependence of power on sample size and probability of death}
<<powerplot1, echo=FALSE, eval=TRUE,out.width='7cm',out.height='7cm'>>=
...
\begin{frame}[label=slide12b,fragile] %slide 12b
\frametitle{Application to clinical problem - Dependence of power on sample size and probability of death}
<<powerplot2, echo=FALSE, eval=TRUE,out.width='7cm',out.height='7.5cm'>>=
source("PowerAndMoments.R") #include functions to compute moments and power
# explore power for specific situation described above, varying the
# probabiliy of death
# sample size in each group
...
# To dos:
# place all four graphs nicely on one or two slides
# adapt legend (lines correpond to $p_0$, mention somewhere that $p_1 = RR * p_0$)
# The value of RR should be displayed somewhere
par(mfrow = c(2, 2)) # construct panel of 2x2 graphs

for (RR in c(1, 1.2, 1.75, 2.5)) {
for (p in chosen_ps) {
  MyPower <- power_gr(alpha = 0.0294, 
                      p_0_0 = p, p_1_0 = RR*p, p_0_1 = p, p_1_1 = p, 
                      mu_0_0 = 0.3, mu_1_0 = 0.25, mu_0_1 = 0.3, mu_1_1 = 0.3, 
                      sigma = 0.1, tau = 1,
                      n_0 = n_0, n_1 = n_1,
                      verbose = FALSE)
...
legend("bottomright", title = TeX("$p_1 = $ RR $ \\cdot p_0$"), 
       legend = as.character(chosen_ps), inset = 0.05, bty = "n", lwd = 2, 
       lty = my.lt, col = my.col )
title(main = paste0("RR = ", as.character(RR)),
      sub = "(1:2 allocation)")
}
@
\end{frame}
...
% ---------------------------------------------------------------------------
% The following frame was experimental to test how to display graph and table
% ---------------------------------------------------------------------------
% \begin{frame}
...
% 
% \column{0.6\textwidth}
% <<powerplot01, echo=FALSE, eval=TRUE,out.width='7cm',out.height='7cm'>>=
% source("PowerAndMoments.R") #include functions to compute moments and power
% # explore power for specific situation described above, varying the
% # probabiliy of death
% # sample size in each group
...
% 
% \column{0.4\textwidth}
% <<xtable01, echo=FALSE, results="asis">>=
% results <- data.frame(chosen_ps, epsilon_ps, samplesize_80)
% 
% colnames(results) <- c("p", "$\\epsilon$","$n_{total}$")
% print(xtable(results, digits = c(0, 2, 3, 0)),
...
# To dos:
# Table should contain columns corresponding to sample sizes needed for each choice of $p_1$
results <- data.frame(chosen_ps, epsilon_ps, samplesize_80)
...
\begin{frame}[label = slide14] %slide 14
