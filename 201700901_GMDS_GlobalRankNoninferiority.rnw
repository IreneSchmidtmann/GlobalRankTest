...
colnames(results[[1]]) <- c("$p_0$", "$\\epsilon$","$n_{total}$")
...
colnames(results[[2]]) <- c("$p_0$", "$\\epsilon$","$n_{total}$")
...
colnames(results[[3]]) <- c("$p_0$", "$\\epsilon$","$n_{total}$")
...
colnames(results[[4]]) <- c("$p_0$", "$\\epsilon$","$n_{total}$")
...
\item If relative risk is small (hazard ratio $\leq$ 1.764 in clinical example)
\begin{itemize}
