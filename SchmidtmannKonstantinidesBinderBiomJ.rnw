...
of death-censored observations}
  ...
  In section 2, we present the application, a clinical trial in pulmonary
  embolism, that motivated our methodological work. The power formulae are
  derived in section 3. Section 4 illustrates the application of the derived
  formula to the planned clinical trial. A discussion and concluding remarks
  are provided in Section 5.


  \section{Thrombolysis application}
  \textit{Prof. Konstantinides: please change, extend, shorten or generalize as
  ...
  in the reference group with standard treatment (group 0) and $X_{11}, \ldots, X_{1n_1}$
  the values of the quantitative endpoint in the group with the new experimental
  treatment (group 1). The event times in group $i$ are given by
  $T_{i1}, \ldots, T_{in_i}$. Let $D_{ik} = 1$ indicate that subject $k$ in
  group $i$ dies before the quantitative endpoint $X_{ik}$ can be determined,
  i. e. $D_{ik} = 1$ if $T_{ik} < \tau $, $D_{ik} = 0$ otherwise. The survival
  probability in group $i$ up to time $\tau$ is given by $q_i = S_i(\tau)$.
  Accordingly, the cumulative mortality in group $i$ up to time $\tau$ is given
  by $p_i = 1 - q_i = P(D_{ik} = 1)$.
  ...
  Following Matsouka and Betensky (2015), we introduce a new variable
  $\widetilde{X}_{ik} = D_{ik}(\eta  + T_{ik}) + (1 - D_{ik})X_{ik}$ with
	...
	have lower values then the patients who survive to have the quantitative
	endpoint $X_{ik}$ determined. The patient that dies first has the lowest
	value, then the other deseased patients follow according to their times
	of death. For those patients who survive, $\widetilde{X}_{ik}$ is the observed
	value of the quantitative endpoint. The $\widetilde{X}_{ik}$ are used to
	determine ranks and to compute the Wilcoxon-Mann-Whitney test statistic
	$ U = (\sum_{k=1}^{n_0}
	  \sum_{l=1}^{n_1}I(\widetilde{X}_{0k} < \widetilde{X}_{il})) / n_0 n_1  $
