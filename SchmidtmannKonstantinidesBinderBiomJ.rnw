...
\newcommand{\J}{J\"{o}reskog}
  \newcommand{\So}{S\"{o}rbom}fu
  \newcommand{\bcx}{{\bf X}}
  \newcommand{\bcy}{{\bf Y}}fu
  \newcommand{\bcz}{{\bf Z}}
  \newcommand{\bcu}{{\bf U}}
  \newcommand{\bcv}{{\bf V}}
  \newcommand{\bcw}{{\bf W}}
...
The power is then given by
  ...
  \begin{align*}
	  1 - \beta &= \Phi\left(z_{\alpha}\frac{\sigma_0(\widetilde{U})}{\sigma_1(\widetilde{U})} +
                ...
                p^2 \left[ 3 +(n_0 + n_1 -2) p \right]} \right)
							%\varepsilon\sqrt{\frac{12 n_0 n_1}{\left\{ (n_0 + n_1 +1) - p^2 \left[ 3 +(n_0 + n_1 -2) p \right]} \right\}}}
							%
              %&= \Phi\left(z_{\alpha} \sqrt{\frac{12 \big\{ \pi_{\widetilde{U}1} (1 - \pi_{\widetilde{U}1}) + \right.
            %                         (n_0 - 1) (\pi_{\widetilde{U}2} - \pi_{\widetilde{U}1}^2 - p_0^2 p1 / 12) +
            %                         (n_1 - 1) (\pi_{\widetilde{U}3} - \pi_{\widetilde{U}1}^2 - p_0 p1^2 / 12) - p_0 p_1 / 4 \big\}}{(n_0 + n_1)(n_0 + n_1 +1)}} \right.\\
            % &{\hphantom{= \Phi\bigg(}} \left.
            % + \varepsilon \sqrt{\frac{12 n_0 n_1 }{(n_0 + n_1 + 1 - p^2 \left[ 3 +(n_0 + n_1 -2) p \right])}} \right)
  \end{align*}
  ...
  We explore the power as a function of sample size and the probabilities of death in both treatment
	...
	In a first step we determine the non-parametric non-inferiority margin for the case of
	no censoring by death
