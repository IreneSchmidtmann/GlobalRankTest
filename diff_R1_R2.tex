...
%DIF DEL SchmidtmannKonstantinidesBinderBiomJ-R1.tex   Wed Jul 25 01:26:13 2018
%DIF ADD SchmidtmannKonstantinidesBinderBiomJ-R2.tex   Wed Jul 25 22:44:27 2018
...
\begin{document}
%DIF <  1st revision of paper submitted to Biometrical Journal
%DIF >  2nd revision of paper submitted to Biometrical Journal

%DIF <  initial chunk that loads necessary libraries and functions
%DIF >  initial chunk that loads necessary libraries
...
\Year{2010}
\pagespan{1}{}
\DIFdelbegin %DIFDELCMD < \keywords{Censoring by death; Global rank test; Non-inferiority; Combined endpoints; Pulmonary embolism;\\
...
%DIFDELCMD < }  %%%
\DIFdelend \DIFaddbegin \keywords{Censoring by death; Global rank test; Non-inferiority;
...
suppinfo}.
}  \DIFaddend %%% semicolon and fullpoint added here for keyword style
...
\DIFdelbegin \DIFdel{,
}\DIFdelend consider power analysis in a randomized clinical trial setting in which the goal
...
%DIF < % If there is not enough space inside the running head
%DIF < % for all authors including the title you may provide
%DIF < % the leftmark in one of the following three forms:
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < % \renewcommand{\leftmark}
%DIF < % {First Author: A Short Title}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < % \renewcommand{\leftmark}
%DIF < % {First Author and Second Author: A Short Title}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < % \renewcommand{\leftmark}
%DIF < % {First Author et al.: A Short Title}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < % \tableofcontents  % Produces the table of contents.
%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{$S(t)$ }\DIFdelend \DIFaddbegin \DIFadd{$S_{i}(t)$ }\DIFaddend is the survival function. Accordingly, the cumulative mortality in
...
If assumptions about the distribution of \DIFaddbegin \DIFadd{the }\DIFaddend quantitative endpoint and event time
...
\begin{align*}
  1 - \beta &= \text{P} \left(\frac{U - \mu_0(U)}{\sigma_0(U)}  >
            ...
            &= \Phi\left(z_{\alpha}\frac{\sigma_0(U)}{\sigma_1(U)} +
                            \frac{\mu_1(U) - \mu_0(U)}{\sigma_1(U)} \right) \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
\DIFaddbegin \DIFadd{where $\Phi$ denotes the distribution function of the standard normal
distribution.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < {\tt{R 3.4.4}}%%%
...
%DIF <  Wed Jul 25 01:23:14 2018
%DIF >  Wed Jul 25 22:44:26 2018
\begin{table}[ht]
\centering
...
\DIFaddend In our example there is little difference in power between the tied case and
...
probability of death is identical in both treatment groups. \DIFdelbegin \DIFdel{The
values are displayed }\DIFdelend \DIFaddbegin \DIFadd{Values for the tied
case corresponding to the situation depicted in figure \ref{fig:powerplot_untied}
are shown }\DIFaddend in table \ref{tab:AppendixTied} in the appendix.
...
RR & $p_0$ & $n$ & untied: $\beta = 0.8$ & $\beta = 1.0$ & $\beta = 1.2$ & tied: $\beta = 0.8$ & $\beta = 1.0$ & $\beta = 1.2$ \\ 
  \hline
1 & 0.00 & 147 & \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddendFL 0.80 & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & 0.81 \DIFdelbeginFL %DIFDELCMD < & %%%
\DIFdelFL{0.81 }\DIFdelendFL \\ 
    & 0.01 & 153 & \DIFdelbeginFL \DIFdelFL{0.78 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.79 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddendFL 0.80 & \DIFdelbeginFL \DIFdelFL{0.83 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.78 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL \\ 
    & 0.02 & 162 & \DIFdelbeginFL \DIFdelFL{0.82 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & \DIFdelbeginFL \DIFdelFL{0.83 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.79 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & \DIFdelbeginFL \DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddFL{0.81 }& \DIFaddFL{0.81 }\DIFaddendFL \\ 
    & 0.05 & 186 & 0.80 & \DIFdelbeginFL \DIFdelFL{0.81 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.80 }& \DIFaddendFL 0.81 & 0.80 & \DIFdelbeginFL \DIFdelFL{0.82 }%DIFDELCMD < & %%%
\DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL \\ 
  ...
  1.2 & 0.00 & 147 & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & 0.80 & \DIFdelbeginFL \DIFdelFL{0.83 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & 0.80 & \DIFdelbeginFL \DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddFL{0.80 }\DIFaddendFL \\ 
    ...
    & 0.10 & 204 & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.78 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & 0.81 & \DIFdelbeginFL \DIFdelFL{0.78 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }& \DIFaddFL{0.80 }\DIFaddendFL \\ 
    & 0.20 & 276 & \DIFdelbeginFL \DIFdelFL{0.78 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & 0.80 & 0.80 & \DIFdelbeginFL \DIFdelFL{0.81 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 \DIFaddbeginFL & \DIFaddFL{0.81 }\DIFaddendFL \\ 
  1.75 & 0.00 & 147 & \DIFdelbeginFL \DIFdelFL{0.80 }%DIFDELCMD < & %%%
\DIFdelendFL 0.81 & 0.81 & 0.80 & \DIFdelbeginFL \DIFdelFL{0.80 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.79 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddFL{0.80 }\DIFaddendFL \\ 
    ...
    & 0.02 & 147 & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.81 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & 0.81 & 0.80 & 0.81 \DIFaddbeginFL & \DIFaddFL{0.80 }\DIFaddendFL \\ 
    & 0.05 & 147 & \DIFdelbeginFL \DIFdelFL{0.81 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & 0.80 & 0.80 & \DIFdelbeginFL \DIFdelFL{0.79 }%DIFDELCMD < & %%%
\DIFdelendFL 0.81 \DIFaddbeginFL & \DIFaddFL{0.80 }& \DIFaddFL{0.80 }\DIFaddendFL \\ 
  ...
  2.5 & 0.00 & 147 & \DIFdelbeginFL \DIFdelFL{0.81 }%DIFDELCMD < & %%%
\DIFdelendFL 0.80 & 0.81 & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddendFL 0.80 \\ 
    ...
    & 0.02 & 135 & \DIFdelbeginFL \DIFdelFL{0.80 }%DIFDELCMD < & %%%
\DIFdelendFL 0.81 & \DIFdelbeginFL \DIFdelFL{0.82 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & 0.80 & \DIFdelbeginFL \DIFdelFL{0.78 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.80 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.84 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }& \DIFaddFL{0.80 }\DIFaddendFL \\ 
    ...
    & 0.20 & 60 & \DIFdelbeginFL \DIFdelFL{0.80 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.81 }\DIFaddendFL & 0.80 & \DIFdelbeginFL \DIFdelFL{0.82 }%DIFDELCMD < & %%%
...
\DIFdelbegin \DIFdel{80.4 }\DIFdelend \DIFaddbegin \DIFadd{80.3 }\DIFaddend \% on average, range
\DIFdelbegin \DIFdel{77.9 }\DIFdelend \DIFaddbegin \DIFadd{79.5 }\DIFaddend \% - \DIFdelbegin \DIFdel{83.5 }\DIFdelend \DIFaddbegin \DIFadd{81.3 }\DIFaddend \%.
In the tied case we observed average power
80.4 \%, range
\DIFdelbegin \DIFdel{77.6 }\DIFdelend \DIFaddbegin \DIFadd{79.5 }\DIFaddend \% - \DIFdelbegin \DIFdel{83.6 \%.
}\DIFdelend \DIFaddbegin \DIFadd{81.1 \%.
If the power is 80 \%, i. e. 0.8, with }\ensuremath{10^{4}} \DIFadd{replicates, the standard error
of the estimated power is 0.004, i. e.
0.4 percentage points. This supports the conclusion
that deviations of the observed power from 80 \% are not substantinal but due to
random noise.
}\DIFaddend 
...
relative risk in the new treatment group \DIFdelbegin \DIFdel{is not too high }\DIFdelend compared to the reference group \DIFaddbegin \DIFadd{is not
too high}\DIFaddend . This effect is most pronounced if the probability of death is identical
...
to smaller values in the reference distribution. If the \DIFdelbegin \DIFdel{relative }\DIFdelend risk of death in the new
treatment group is much higher  \DIFdelbegin \DIFdel{relative to }\DIFdelend \DIFaddbegin \DIFadd{than in }\DIFaddend the reference group the effects are
...
\DIFdelbegin %DIFDELCMD < \bibitem[Matsouaka et al (2016) Matsouaka, R. A., Singhal, A. B., Betensky, R. A.]{bib7}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibitem[Matsouaka et al (2018) Matsouaka, R. A., Singhal, A. B., Betensky, R. A.]{bib7}
