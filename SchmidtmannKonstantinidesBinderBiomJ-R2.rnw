...
All calculations were performed using \Sexpr{noquote(sessionInfo.txt$R.version$version.string)},
...
corresponding to the situation depicted in \ref{fig:powerplot_untied} are shown in
table \ref{tab:AppendixTied} in the appendix.
...
total sample size was chosen as determined in table \ref{tab:test}. \Sexpr{r} datasets
...
treatment group is much higher relative than in the reference group the
