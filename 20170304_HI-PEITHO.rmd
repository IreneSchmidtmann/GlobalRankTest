...
HR <- 1                     # hazard ratio
delta_0 <- epsilon          # mean difference (mu_0 - mu_1) under non-inferiority null hypothesis H0
delta_1 <- 0                # assumed mean difference  (mu_0 - mu_1) under (non-inferiority) alternative hypothesis H1
...
$\mu_1$       | `r mu_1 `    | mean in treatment group under (non-inferiority) null hypothesis
...
$\frac{\mu_0 - \mu_1}{\sigma}$ | `r delta_0/sigma ` | effect size unter null hypothesis
$\delta_1$    | `r delta_1 ` | assumed mean difference  ($\mu_0 - \mu_1$) under (non-inferiority) alternative hypothesis
...
So $p_i = P(d_{ik} = 1) = P(t_{ik} < \tau)= F_i(\tau), q_i = S_i(\tau), i=0, 1$ and 

Formula by Matsouaka and Betensky:
