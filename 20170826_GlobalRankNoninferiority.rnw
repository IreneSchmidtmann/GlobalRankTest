...
multiplier <- 0.5 - pnorm(-1/sqrt(8))
p <- c(0, 0.01, 0.02, 0.05, 0.1, 0.2)
epsilon <- (1 - p)^2 * multiplier
results <- data.frame(p, epsilon)
library(xtable)
print(xtable(results, digits = c(0, 2, 4)), include.rownames = FALSE)
@
\end{frame}
...
\end{frame}
    ...
    % \item $\sigma_1^2(U)$ variance under $H_1$, specifically for 
    %       $ \mu(U) = P(\tilde{X_{0k}} < \tilde{X_{1l}}) = \frac{1}{2}$\\
...
<<Figure1, echo=FALSE, out.width='0.4\\textwidth', out.height = '0.4\\textheight'>>=
plot(x)
# print(x)
@
\end{frame}
...
<<boring-plots-part1,echo=FALSE,eval=TRUE,out.width='4cm',out.height='4cm'>>=
par(las=1,mar=c(4,4,.1,.1))  # tick labels direction
boxplot(x) 
@
    \end{column}
    \begin{column}{.5\linewidth}
<<boring-plots-pat2,echo=FALSE,eval=TRUE,out.width='4cm',out.height='4cm'>>=
par(las=1,mar=c(4,4,.1,.1))  # tick labels direction
...
<<boring-plots,fig.show='hold', fig.width=3, fig.height=3,out.width='0.45\\textwidth'>>=
par(las=1,mar=c(4,4,.1,.1))  # tick labels direction
boxplot(x) 
hist(x,main='',col="blue",probability=TRUE) 
lines(density(x),col="red")
