...
$H_1: P(X_0 < X_1) >  \frac{1}{2} - \tilde{\varepsilon}$.
 ...
 $\tilde{\varepsilon} = 1/2 - \Phi(-1/\sqrt{8})  \approx $ \Sexpr{round(0.5 - pnorm(-1/sqrt(8)), 4)}.
...
RR_column <- character()
for (RR in seq_along(chosen_risks)) {
  results_for_table <- rbind(results_for_table, results[[RR]])
  RR_column <- c(RR_column, as.character(chosen_risks[RR]), rep(" ", l_chosen_ps - 1))
}
results_for_table <- cbind(RR_column, results_for_table)

colnames(results_for_table) <- c("RR", "$p_0$", "$\\varepsilon$","$n_{total}$")
print(xtable(results_for_table,caption = 'Effective non-inferiority margin and total samples size as function of relative risk (RR) of death in the new treatment group (risk $p_1$) compared to the reference group (risk $p_0$) for given $\\alpha = 0.0294$ and power of 80\\% with sample size allocation for reference to new treatment 1:2.',label = "tab:test",digits = c(0, 0, 2, 3, 0)),
